...
void test_crossbar_forward_propagation() {
    // Init 2-input, 2-output routing core
    ...
    // Tick 1: Fire inputs simultaneously
    inputs[0] = true;
    inputs[1] = true;
...
void test_system_learning_loop() {
    ...
    // Tick 1: Stimulate Input 0 only. No error signal.
    // Builds up an eligibility trace inside Synapse[0][0] and Synapse[1][0]
    inputs[0] = true;
    inputs[1] = false;
    core.tick(inputs, outputs, dfa_errors);

    // Tick 2: Stop inputs. Broadcast a global error strictly to Output Channel 0.
    inputs[0] = false;
